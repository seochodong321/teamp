...
export default function CreateProjectModal({ onClose }) {
  ...
  const goNext = async () => {
    ...
    if (step === 1) {
      setLoading(true)
      try {
        const p = await createProject({ name, emoji, purpose, category: finalCategory, startDate, endDate, endTime: endTime || null, roomNames })
        setCreated(p)
        setStep(2)
  ...
  const handleRoomKey = (e) => { if (e.key === 'Enter' && !e.isComposing) { e.preventDefault(); addRoom() } }
          ...
          {step === 1 && (
            <div className={styles.form}>
              ...
              <div className={styles.addRoomRow}>
                <input className={styles.input} value={newRoom} onChange={(e) => setNewRoom(e.target.value)} onKeyDown={handleRoomKey} placeholder="채팅방 이름 입력 (Enter)" />
          ...
          {step === 2 && created && (
            <div className={styles.done}>
              ...
              <div className={styles.linkBox}>
                ...
                <button className={styles.linkCopy} onClick={() => { navigator.clipboard.writeText(inviteLink); alert('복사됐어요!') }}>복사</button>
              </div>
