    ...
    <div className={styles.page}>
      ...
      <div className={styles.messages} ref={messagesRef} onScroll={handleMessagesScroll}>
        ...
        {visibleMessages.map((msg, index) => {
          ...
          // 텍스트·이미지·파일 말풍선 — 래퍼 공통, 내용물만 타입 분기 (MessageBubble)
          return (
            <React.Fragment key={msg.id}>
              {showDateDivider && <div className={styles.dateDivider}><span>{formatDateLabel(msgDate)}</span></div>}
              <MessageBubble msg={msg} isMine={isMine} isGrouped={isGrouped}
                avatarEl={avatarEl} nameEl={nameEl} timeEl={timeEl} readCount={readCount}
                myId={currentUser?.id}
                onToggleReaction={(key) => toggleReaction(roomId, msg.id, key)}
                onImageClick={() => setLightbox({ url: msg.fileUrl, name: msg.text })} />
            </React.Fragment>
          )
        })}
        <div ref={bottomRef} />
