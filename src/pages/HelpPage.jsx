...
const sections = [
  ...
  },
  {
    ...
    steps: [
      ...
      { label: '소속·연락처', desc: '소속과 이메일을 등록하면 팀원들이 내 프로필을 확인할 때 함께 표시돼요. 프로필을 저장하면 함께한 모든 프로젝트에 자동 반영돼요.' },
      { label: '팀프폴리오 생성하기', desc: '프로필 페이지 하단 [팀프폴리오 관리]에서 [생성하기]를 누르면 나만의 공개 포트폴리오가 만들어져요. 꽃다발·프로젝트 이력·통계 섹션을 켜고 끌 수 있어요.' },
      { label: '반영하기로 공개', desc: '커스텀이 끝나면 [반영하기] 버튼을 눌러야 외부 링크(팀프폴리오)에 적용돼요. 언제든 수정 후 다시 반영할 수 있어요.' },
  ...
  },
  {
    ...
    steps: [
      ...
      { label: '쪽지 보내기', desc: '프로필 모달에서 [쪽지 보내기]를 누르면 쪽지함으로 이동해 메시지를 전달할 수 있어요. 프로젝트가 끝난 후에도 자유롭게 연락할 수 있어요.' },
      { label: '새 프로젝트로 초대하기', desc: '내가 리더로 있는 활성 프로젝트가 있다면 [+ 내 프로젝트로 초대하기] 버튼이 나타나요. 프로젝트를 선택하면 해당 프로젝트 페이지로 이동해 멤버를 추가할 수 있어요.' },
  ...
  },
  {
    ...
    title: '쪽지함',
    steps: [
      { label: '쪽지란?', desc: '쪽지는 채팅과 달리 한 장의 메모처럼 상대에게 전달하는 짧은 메시지예요. 실시간 대화가 부담스러울 때 쪽지로 조용히 연락해보세요.' },
      { label: '쪽지 보내기', desc: '쪽지함 오른쪽 위 [✏️ 쪽지 쓰기]를 누르고, @아이디로 받는 사람을 검색해 내용을 작성하면 돼요. 팀프 매치에서 [문의하기]를 누르면 쪽지함이 자동으로 열려요.' },
      { label: '받은 쪽지 읽기', desc: '쪽지함 왼쪽에서 받은 쪽지나 보낸 쪽지를 선택하면 보낸 이, 받는 이, 날짜, 내용을 편지처럼 확인할 수 있어요.' },
      { label: '답장하기', desc: '쪽지를 읽은 후 [↩ 답장하기] 버튼을 눌러 답장을 작성하세요. 같은 쪽지에 대화 흐름이 기록돼요.' },
    ],
  },
  {
    id: 'calendar',
  ...
  },
  {
    ...
    steps: [
      { label: '팀프폴리오란?', desc: '팀프폴리오는 함께한 프로젝트 이력, 받은 꽃다발(피드백), 팀 통계를 외부에 공개할 수 있는 나만의 포트폴리오 페이지예요. teamp.kr/u/아이디 형태의 링크로 누구에게나 공유할 수 있어요.' },
      { label: '생성 & 커스텀', desc: '프로필 페이지 하단 [팀프폴리오 관리]에서 [생성하기]를 눌러 시작해요. 꽃다발·프로젝트 이력·통계 섹션을 켜고 끄면서 보여줄 내용을 직접 고를 수 있어요.' },
      { label: '반영하기', desc: '편집이 끝나면 반드시 [반영하기]를 눌러야 외부 링크에 적용돼요. 반영하기 전까지는 공개 페이지에 변경 사항이 나타나지 않아요.' },
      { label: '꽃다발이 핵심이에요', desc: '받은 꽃다발 태그는 함께한 팀원들이 랩업 피드백에서 남겨준 기억이에요. 이 사람이 어떤 방식으로 기여했는지를 가장 잘 보여주는 데이터예요.' },
      { label: '링크 활용', desc: '채용 담당자나 새로운 협업 파트너에게 팀프폴리오 링크를 보내 나의 팀 경험과 기여 방식을 직접 보여주세요.' },
    ],
  },
]
