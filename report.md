...
> **처리 현황 (2026-06-12, 커밋 `23762ad`)**: 🔴 1·3·4번 + 🟡 6·8번 완료(P0 서버 집계 함수·rooms 접근 규칙·알림 발신자 검증·wrapups 멤버 축소·KST 날짜). 규칙·함수 배포됨. 단, 클라 변경은 **push→Vercel 배포 후** 실효. 2번(기존 피드백 유실 확인)·5번(Sentry, 계정 필요)은 유저 액션 대기. 레거시 fallback·구버전 알림 임시 허용 조항은 전 클라 배포 후 제거할 것.
